...
class DiscordUtils():
    ...
    @staticmethod
    async def get_bot_emoji(bot, name):
        """
        Get custom emoji from bot server
        """
        server = discord.utils.get(bot.guilds, name='/dev/*.py')
    ...
    async def get_user(bot, ctx, arg, nick=None):
        ...
        if nick is None:
            ...
            if message:
                ...
                if not name.isdigit():
                    return await ctx.send("Используйте только числа")
                flag = True
